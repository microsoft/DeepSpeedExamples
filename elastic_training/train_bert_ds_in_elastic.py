...
def get_create_experiment_dir_path(checkpoint_dir: pathlib.Path)-> pathlib.Path:
    current_time = datetime.datetime.now(pytz.timezone("US/Pacific"))
    expname = "bert_pretrain.{0}.{1}.{2}".format(
        current_time.year,
        current_time.month,
        current_time.day,
    )
...
def create_experiment_dir(checkpoint_dir: pathlib.Path,
                          all_arguments: Dict[str, Any], device_id) -> pathlib.Path:
...
def train(
    ...
    print(f"=> set cuda device = {local_rank} Global Rank = {global_rank}")

    device = (torch.device("cuda", local_rank) if (local_rank > -1)
              and torch.cuda.is_available() else torch.device("cpu"))
    torch.cuda.set_device(device)
    exp_dir = get_create_experiment_dir_path(pathlib.Path(checkpoint_dir))

    data_iterator = InfiniteIterator(data_iterator)

    dist.init_process_group(
        backend='nccl', init_method='tcp://worker-0:23456'
    )
    ...
    dist.broadcast_object_list(exp_dir_exists, src=0)

    if checkpoint_dir and exp_dir_exists[0]==False:
        ...
        exp_dir = create_experiment_dir(checkpoint_dir, all_arguments, global_rank)
        log_dist(f"Experiment Directory created at {exp_dir}",
                 ranks=[0],
                 level=logging.INFO)
    elif exp_dir.exists()==True:
        log_dist("Loading from Experiment Directory",
                 ranks=[0],
                 level=logging.INFO)
        load_checkpoint_dir = exp_dir
...
def main():
    ...
    load_checkpoint_dir: str = args.load_checkpoint_dir
    # Dataset Parameters
    mask_prob: float = args.mask_prob
    random_replace_prob: float = args.random_replace_prob
    unmask_replace_prob: float = args.unmask_replace_prob
    ...
    dist.init_process_group(
        backend='nccl', init_method='tcp://worker-0:23455', rank = 0, world_size = 1
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DeepSpeed Elastic HelloDeepSpeed Training")
    parser.add_argument("--checkpoint_dir", default=None, type=str, help="Checkpoint dir")
    parser.add_argument("--load_checkpoint_dir", default=None, type=str, help="Checkpoint load dir")
