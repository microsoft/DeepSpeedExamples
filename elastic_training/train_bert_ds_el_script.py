...
def create_data_iterator(
    ...
    train_sampler = ElasticDistributedSampler(dataset, num_replicas= world_size, rank=rank)
...
def train(
    ...
    """
    log_dist("Starting Script", ranks=[0], level=logging.INFO)
    ...
    dist.init_process_group(
        backend='nccl', init_method="env://", timeout=datetime.timedelta(seconds=100)
    )
    dist.barrier()
    log_dist("NCCL Initialized", ranks=[0], level=logging.INFO)
    ...
    ################################
    # Broadcasting from Rank 0 to avoid creation of multiple directories
