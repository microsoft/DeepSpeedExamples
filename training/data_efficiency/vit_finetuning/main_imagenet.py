...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dummy:
        ...
        #transform_train = transforms_imagenet_eval
...
def train(scheduler, train_loader, model, criterion, optimizer, epoch, args):
    ...
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, top1, top5, lr_progress,length,],
...
def validate(val_loader, model, criterion, args):
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                # measure accuracy and record loss
                acc1, acc5 = accuracy(output, target, topk=(1, 5))
                losses.update(loss.clone(), images.size(0))
                top1.update(acc1[0], images.size(0))
                top5.update(acc5[0], images.size(0))
    ...
    return top5.avg, top1.avg, losses.avg
