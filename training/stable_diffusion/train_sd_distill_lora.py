...
def main(args):
    ...
    dataset_hf = load_dataset('poloclub/diffusiondb', '2m_first_50k')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                model_pred = unet(
                    noisy_model_input, timesteps, encoder_hidden_states, cross_attention_kwargs={"scale":1}, class_labels=class_labels
                ).sample

                gc = 7.5
                with torch.no_grad():
                    teacher_cond_noise = unet(noisy_model_input, timesteps, encoder_hidden_states, cross_attention_kwargs={"scale":0}, class_labels=class_labels).sample
                    teacher_uncond_noise = unet(noisy_model_input, timesteps, encoder_hidden_states_uncond, cross_attention_kwargs={"scale":0}, class_labels=class_labels).sample
