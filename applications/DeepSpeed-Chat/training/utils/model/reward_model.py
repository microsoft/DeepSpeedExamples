...
class RewardModel(nn.Module):
    ...
    def forward_group(
        ...
        for i in range(bs):
            # here we do pairwise loss for all combinations of chosen and rejected
            ...
            for j in range(i + 1, bs):
                rejected_id = input_ids[j]

                check_divergence = ((chosen_id !=
                                     rejected_id).nonzero()).view(-1)
                if len(check_divergence) == 0:
                ...
                c_truncated_reward = rewards[i][divergence_ind:end_ind]
        ...
        rejected_mean_scores = torch.stack([torch.ones(1)
                                            for i in range(2)]).to(loss.device)
        return {
            "loss": loss,
            "chosen_mean_scores": chosen_mean_scores,
