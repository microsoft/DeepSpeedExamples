...
default_params = {
        ...
        "sst2": {"max_seq_length": 64,"batch_size":32,"eval_step":200},
        "stsb": {"max_seq_length": 128,"batch_size":32,"eval_step":50},
        "qqp": {"max_seq_length": 128,"batch_size":32,"eval_step":1000},
        "qnli": {"max_seq_length": 128,"batch_size":32,"eval_step":1000},
        "rte": {"max_seq_length": 128,"batch_size":32,"eval_step":50}
    }
...
def parse_args():
    ...
    parser.add_argument("--distill_method", type=str, default="three_stage", help="Where to store the final model.")
    parser.add_argument("--output_dir", type=str, default=None, help="Where to store the final model.")
    parser.add_argument("--pretrained_dir_student", type=str, default=None, help="Where to load the pretrained model.")
    parser.add_argument("--pretrained_dir_teacher", type=str, default=None, help="Where to load the pretrained model.")
    ...
    parser.add_argument("--save_best_checkpoint", action="store_true", help="save best checkpoint model") 
    parser.add_argument("--save_last_model", action="store_true", help="save the last model") 
    parser.add_argument("--clean_last_model", action="store_true", help="clean the last model") 
...
def main():
    ...
    if args.distill_method != 'zero_stage':
        ...
        teacher_config = AutoConfig.from_pretrained(args.model_name_or_path_teacher, num_labels=num_labels, finetuning_task=args.task_name)
    ...
    def replace_config(model_tmp,label_to_id = None):
        if (
            model_tmp.config.label2id != PretrainedConfig(num_labels=num_labels).label2id
            and args.task_name is not None
            and not is_regression
        ):
            # Some have all caps in their config, some don't.
            label_name_to_id = {k.lower(): v for k, v in model_tmp.config.label2id.items()}
        ...
        if label_to_id is not None:
            ...
            model_tmp.config.id2label = {id: label for label, id in config.label2id.items()}
        elif args.task_name is not None and not is_regression:
            model_tmp.config.label2id = {l: i for i, l in enumerate(label_list)}
            model_tmp.config.id2label = {id: label for label, id in config.label2id.items()}
    ...
    if args.task_name == "mnli":
        ...
        mm_eval_dataloader = DataLoader(
            mm_eval_dataset, collate_fn=default_data_collator, sampler=mm_eval_sampler, batch_size=args.per_device_eval_batch_size)
    ...
    if args.distill_method != 'zero_stage':
        ...
        teacher_result, _, best_dev_acc, _  = arrange_output(args.task_name, teacher_out, previous_best, 0) 
        print_rank_0(f"teacher model: {teacher_result}")
    else:
        teacher_result = None
    best_dev_acc = 0.0
    stat_history = {"lr1":[],"lr2":[],'train_att_loss':[],'train_ffn_loss':[], 'train_loss':[],'eval':[], }
    
    def train(model, previous_best=None, best_dev_acc=0, text_note='', completed_steps=0,pred_distill=False, intermediate_distill=False):
        loss_mse = MSELoss()
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if intermediate_distill:
                    ...
                    if  layer_reduction_enabled:
                        ...
                        layers_per_block = int(teacher_layer_num / student_layer_num) #2###[1, 3, 5, 7, 9, 11]
                    ...
                    if completed_steps%1000 ==0:
                        print_rank_0 (f"check at step:{completed_steps}, teacher_rep_list_{teacher_layer_num}: {rep_list}, teacher_att_list_{student_layer_num}:{att_list}")    
                ...
                model.backward(loss) #<=======================when using deepspedd engine fp16, we should not use loss.backward()
                model.step()  
                ...
                if completed_steps%args.eval_step==0  and completed_steps>=10:
                    ...
                    print_rank_0 (f"***** Running evaluation Stage {text_note}*****")
                    print_rank_0 ("  {} step of {}".format(completed_steps,args.max_train_steps))
                    model.eval()
                    result = eval(model)
                    model.train()
                    ...
                    current_result, previous_best, best_dev_acc, save_model  = arrange_output(args.task_name, result, previous_best, best_dev_acc)   
                    try:      
                        print_rank_0 ('{'+f"eval_result: {current_result}, step: {completed_steps/args.max_train_steps}, train_loss: {stat_history['train_loss'][-1]}, train_ffn_loss: {stat_history['train_ffn_loss'][-1]},  train_att_loss:{stat_history['train_att_loss'][-1]}, lr1: { stat_history['lr1'][-1]}, lr2: { stat_history['lr2'][-1]}, "+'}')
                    except:
                        print_rank_0 (current_result)
                    if previous_best is not None:
                        print_rank_0 (f"teacher_result: {teacher_result}\nPrevious best = {previous_best}")
        ...
        print_rank_0 (f"Teacher perforamnce = {teacher_result} \n Previous best = {previous_best}")    
    ...
    completed_steps = 0
    if args.distill_method == 'zero_stage':
        previous_best, best_dev_acc, completed_steps, model_engine  =  train(model_engine, previous_best, best_dev_acc, '0/0', completed_steps, pred_distill=False,intermediate_distill=False)
    elif args.distill_method == 'one_stage': 
        previous_best, best_dev_acc, completed_steps, model_engine  =  train(model_engine, previous_best, best_dev_acc, '1/1', completed_steps, pred_distill=True, intermediate_distill=True)
    ...
    if args.save_last_model:
        ...
        current_result, previous_best, best_dev_acc, _  = arrange_output(args.task_name, result, previous_best, best_dev_acc)   
        print_rank_0 (f"Clean last_iter models, and the accuracy of the clean last_iter model is {current_result}")
