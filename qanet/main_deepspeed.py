...
from model_train.model_train_deepspeed import Model_Trainer
import deepspeed
...
def main(args):
    ...
    if torch.cuda.is_available():
        print(f"device is cuda, local rank is {args.local_rank} cuda number is {number_gpu}")
    else:
        print('device is cpu')
    ...
    # 4) deepspeed lr scheduler
    model, optimizer, train_data_loader, scheduler = deepspeed.initialize(args=args,
                                                                          model=model,
                                                                          model_parameters=parameters,
                                                                          training_data=SQuAD_train_dataset,
                                                                          lr_scheduler=None,
                                                                          collate_fn=collate)
