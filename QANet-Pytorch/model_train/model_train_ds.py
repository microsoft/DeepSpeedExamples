...
class Model_Trainer(object):
    ...
    def _save_checkpoint(self, epoch, f1, em, is_best):
        ...
        if not os.path.exists(checkpoint_dir):
            print("Creating deepspeed checkpoint dir: {}".format(checkpoint_dir))
        ...
        if success:
            print('Saving deepspeed checkpoint was successful')
        else:
            print('Saving deepspeed checkpoint was failure')

    def _resume_checkpoint(self, resume_path):
        print('Loading checkpoint: {} ...'.format(resume_path))
        _, checkpoint = self.model.load_checkpoint(resume_path)
        print('deepspeed resume state = {}'.format(checkpoint))
